...
const DecryptionModal = ({
  ...
  const games = {
    ...
    slot: <SlotPassword onSuccess={handleGameSuccess} />,
...
// 第二章：复古拨轮密码锁 - 输入纪念日或生日
const SlotPassword = ({ onSuccess }: { onSuccess: () => void }) => {
  const [digits, setDigits] = useState([0, 0, 0, 0]);
  const [activeSlot, setActiveSlot] = useState(0);
  const [isError, setIsError] = useState(false);
  const [isSuccess, setIsSuccess] = useState(false);
  
  const targetCodes = ['0520', '0607']; // 纪念日 0520 或李丹生日 0607
  ...
  return (
    <div className="text-center space-y-4">
      ...
      >
        ...
        <p className="text-xs text-[#9B6A6C]">提示：你来到这世界的那个夏日，属于你的那颗双子星</p>
      </motion.div>
      ...
      <div className="flex justify-center space-x-3">
        {digits.map((digit, index) => (
          <motion.div
            key={index}
            className={`relative bg-gradient-to-b from-amber-100 to-amber-200 rounded-lg border-2 ${
              activeSlot === index ? 'border-[#E35D6A] shadow-[0_0_15px_rgba(227,93,106,0.5)]' : 'border-amber-300'
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        onClick={checkCode}
        className="px-8 py-3 bg-gradient-to-r from-[#E35D6A] to-rose-500 text-white rounded-xl font-bold shadow-lg"
      >
        {isSuccess ? '解锁成功!' : '转动解锁'}
      </motion.button>
    </div>
