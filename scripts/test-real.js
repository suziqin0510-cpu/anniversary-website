...
(async () => {
  ...
  await page.evaluateOnNewDocument(() => {
    ...
    localStorage.setItem('anniversary-game-state', JSON.stringify({
      unlockedLevels: [1], // Only timeline unlocked, NOT map
      collectedLetters: []
    }));
  ...
  // Inspect slot buttons
  const buttons = await page.$$('button');
  let slotIdx = 0;
  for (let i = 0; i < buttons.length; i++) {
    const parentText = await buttons[i].evaluate(el => el.parentElement?.textContent);
    if (parentText && (parentText.includes('🔒') || parentText.includes('🍲') || parentText.includes('🏔️') || parentText.includes('🐱') || parentText.includes('✈️'))) {
      const disabled = await buttons[i].evaluate(el => el.disabled);
      const cls = await buttons[i].evaluate(el => el.className);
      const innerSpan = await buttons[i].evaluate(el => el.querySelector('span')?.textContent);
      console.log(`Slot ${slotIdx}: disabled=${disabled}, class=${cls}, emoji=${innerSpan}`);
      slotIdx++;
    }
  }
